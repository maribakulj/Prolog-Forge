...
//! filesystem) and therefore cannot live inside `aa-validate` without
...
use aa_rules::Rule;
use aa_validate::{Diagnostic, Severity, StageReport, ValidationContext, ValidationStage};

use aa_csm::LanguageAnalyzer;
...
impl ValidationStage for RuleStage {
    ...
    fn validate(&self, ctx: &ValidationContext<'_>) -> StageReport {
        ...
        if let Err(e) = aa_rules::evaluate(&self.rules, &mut graph) {
            diags.push(Diagnostic {
                severity: Severity::Error,
                file: None,
...
mod tests {
    use super::*;
    use aa_rules::parse as parse_rules;
...
/// Recursively copy `src` into `dst`, skipping `.aye-aye`, `target`,
...
fn mirror_dir(src: &Path, dst: &Path) -> std::io::Result<()> {
    ...
    for entry in walkdir::WalkDir::new(src)
        ...
        .filter_entry(|e| {
            ...
            name != ".aye-aye" && name != "target" && name != ".git"
        })
    {

...
//!
//! Phase 1 step 1 wires exactly one analyzer (Rust, via `pf-lang-rust`); the
//! dispatch shape is already extensible — a new language is a new arm in
//! `analyze_file` plus a new crate under `pf-lang-*`.

use std::fs;
use std::path::Path;

use pf_csm::{CsmFragment, LanguageAnalyzer};
use pf_graph::GraphStore;
use pf_ingest::{walk, IngestOptions, SourceFile};
use pf_lang_rust::RustAnalyzer;
use pf_protocol::IndexingError;

...
use pf_ra_client::{ClientError, DocumentUri, Session, TextEdit, WorkspaceEdit};

use pf_patch::{TypedRenameError, TypedRenameRequest, TypedRenameResolver};
...
impl RaSessionPool {
    ...
    /// key is caller-provided; pf-core passes the workspace's
...
/// Adapter so the pool plugs into `pf-patch::apply_plan_with_resolver`.
/// The resolver's `resolve` signature has no `session_key` field —
/// pf-core extracts one from the caller's workspace root before
/// delegating here.
...
// --- Duplicated from pf-patch::typed_rename to avoid cyclic
// --- deps: applying a WorkspaceEdit is a small pure function, and
// --- keeping it here lets pf-core own the full pool flow.
...
fn apply_text_edits(source: &str, edits: &[TextEdit]) -> Result<String, TypedRenameError> {
    ...
    for pair in byte_edits.windows(2) {
        if pair[1].0 < pair[0].1 {
            return Err(TypedRenameError::InvalidEdit(
                pf_ra_client::Range {
                    start: pf_ra_client::Position {
                        line: 0,
                        character: 0,
                    },
                    end: pf_ra_client::Position {
                        line: 0,
                        character: 0,
                    },
                },

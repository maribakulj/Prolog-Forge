//! Compose evidence from the four Prolog Forge subsystems into a single
//! [`Explanation`].
//!
//! The builder is intentionally pure: all sources of evidence are passed in
//! explicitly, so the caller (typically `pf-core`) retains ownership of the
//! filesystem and the LLM lifecycle. This keeps `pf-explain` trivial to
...
use pf_rules::{trace_derivations, Rule};
...
/// builder does not need to depend on `pf-validate`.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn rule_activation_captured_when_head_touches_anchor() {
        ...
        let _ = pf_rules::evaluate(&program.rules, &mut g).unwrap();
        let anchors = vec!["id_add".to_string()];
        let out = build(ExplainInput {
            plan_label: "t",
            anchors: &anchors,
